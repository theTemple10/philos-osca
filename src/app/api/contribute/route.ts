...
import { createPullRequest, forkRepository, createBranch, waitForForkReady } from "@/lib/github/pr";
import { getRepoTree, getFileContents } from "@/lib/github/content";
import { rankCandidateFiles } from "@/lib/github/relevance";
import { fetchRepoDetails, fetchRepoLanguages } from "@/lib/github/repos";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.action === "generate") {
      ...
      const codeResult = (await (async () => {
        const [tree, repoDetails, repoLanguages] = await Promise.all([
          getRepoTree(token, contribution.targetRepoOwner, contribution.targetRepoName),
          fetchRepoDetails(token, contribution.targetRepoOwner, contribution.targetRepoName),
          fetchRepoLanguages(token, contribution.targetRepoOwner, contribution.targetRepoName),
        ]);

        const candidatePaths = rankCandidateFiles(tree, {
          title: contribution.issueTitle || "",
          body: contribution.issueBody || "",
        });
        const paths = [...new Set(candidatePaths)].slice(0, 12);
        const relevantFiles = await getFileContents(
          token,
        ...
        return generateContributionCode(
          userId,
          {
            title: contribution.issueTitle || "",
            body: contribution.issueBody || "",
      ...
      // Store generated code in a draft PR record for later use during submit
      const branchName = `oss-contributor/${Date.now()}`;
      await prisma.pullRequest.create({
        data: {
          userId,
          contributionId: parsed.contributionId,
          title: codeResult.prTitle || `feat: ${contribution.issueTitle || "contribution"}`,
          body: codeResult.prBody || "",
          branchName,
          generatedCode: {
    ...
    if (body.action === "submit") {
      ...
      // Find the draft PR with generated code
      const draftPr = await prisma.pullRequest.findFirst({
        where: {
          contributionId: parsed.contributionId,
          status: "draft",
        },
      ...
      // Extract files from the stored generated code
      const generatedCode = draftPr?.generatedCode as {
      ...
      const pr = await createPullRequest(token, {
        ...
        title: generatedCode?.prTitle || draftPr?.title || `feat: ${contribution.issueTitle || "contribution"}`,
        body: [
          `## Changes`,
          contribution.issueNumber ? `Closes #${contribution.issueNumber}` : "",
          "",
          `## Description`,
          generatedCode?.prBody || contribution.suggestedApproach || "Automated contribution generated by OSS Contributor.",
          "",
          `---`,
          `*Generated by [OSS Contributor](https://philos-osca.vercel.app)*`,
        ].filter(Boolean).join("\n"),
      ...
      return NextResponse.json({ pullRequest: draftPr || { id: pr.id }, prUrl: pr.html_url });
    }
